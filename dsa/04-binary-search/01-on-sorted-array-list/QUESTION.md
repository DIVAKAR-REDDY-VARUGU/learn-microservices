# Binary Search

> **Pattern:** Binary Search → **On Sorted Array/List**  ·  [📖 Learn this pattern](../../README.md#on-sorted-arraylist)

## Problem

Given an array of integers `nums` which is sorted in **ascending order**, and an integer `target`, write a function to search for `target` in `nums`. If `target` exists, return its index. Otherwise, return `-1`.

You must write an algorithm with `O(log n)` runtime complexity.

**Constraints:**
- `1 <= nums.length <= 10^4`
- `-10^4 < nums[i], target < 10^4`
- All the integers in `nums` are **unique**.
- `nums` is sorted in ascending order.

## Examples

**Example 1**
- Input: `nums = [-1, 0, 3, 5, 9, 12]`, `target = 9`
- Output: `4` (because 9 exists in nums and its index is 4)

**Example 2**
- Input: `nums = [-1, 0, 3, 5, 9, 12]`, `target = 2`
- Output: `-1` (because 2 does not exist in nums)

**Example 3**
- Input: `nums = [5]`, `target = 5`
- Output: `0`

## Your task

Implement the method in **`Answer.java`**, then run `java Answer.java`:

```java
public int search(int[] nums, int target) { /* your code */ }
```
